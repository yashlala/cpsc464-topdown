...
from das_framework.ctools.paths import mkpath
...
def das_productionp(arg=None):
    if os.getenv("DAS_TIER") == "PRD":
        return True
    elif os.getenv("DAS_PRODUCTION_MODE") == "TRUE":
        return True
    else:
        return False

# Utility algorithms/functions

def mergedicts(dict_list: List[Dict]) -> Dict:
    """
...
def transpose_pairs(tuple_list: List[tuple]) -> List[tuple]:
    """
...
def addslash(directory: str) -> str:
    if directory[-1] != '/':
...
def getSafeQueryname(queryname: str) -> str:
    """
...
def getStandardQueryname(safe_queryname: str) -> str:
    """
...
def getMasterIp() -> str:
    ...
    """
    return census_getenv(CC.MASTER_IP)


def isodate(sep: str = "-") -> str:
...
def pretty(item, indent: int = 4, join_with: str = '\n') -> str:
    if type(item) == dict:
...
def printList(thelist: List, join_with: str = "\n\n") -> None:
...
def loadConfigFile(path: str) -> ConfigParser:
    config = ConfigParser()
...
def getGeoDict(config: ConfigParser) -> Dict:
...
def saveNestedListAsTextFile(path: str, thelist: List):
    thelist = flattenList(thelist)
    saveListAsTextFile(path, thelist)


def saveListAsTextFile(path: str, thelist: List, mode: str = "w"):
    if isS3Path(path):
...
def saveConfigFile(path: str, config: ConfigParser):
...
def saveJSONFile(path: str, data, indent: int = None):
    if isS3Path(path):
        savefile = s3.s3open(path=path, mode="w")
...
def expandPathRemoveHdfs(path: str) -> str:
    return os.path.expandvars(path).replace(CC.HDFS_PREFIX,"")


def savePickledRDD(path: str, rdd, *, dvs_singleton=None) -> None:
...
def savePickleFile(path: str, data) -> None:
...
def repartitionNodesEvenly(nodes_rdd, num_parts, modified_geoids_dict, is_key_value_pairs=False, partition_size=0):
    ...
    :param sorted_block_ids: sorted list of block geocodes
    :param geoid_len: length of geocode attribute of each node
    :param is_block: indicates if the nodes_rdd corresponds to the block geolevel
    ...
    # Redefine num_parts and partition_size if needed to ensure there are at least min(16, rdd_count) partitions:
    min_partitions = min(16, rdd_count)
...
def partitionByParentGeocode(block_nodes, num_partitions: int = None):
...
def partitionByGeocode(block_nodes, num_partitions: int = None):
    """ Partition an RDD of geonodes by parent geocode """
    return block_nodes \
...
def splitBySize(code_count_list: List) -> Dict:
...
def partitionBySiblingNumber(block_nodes, num_partitions: int):
...
def rddPartitionDistributionMoments(rdd) -> str:
...
def ship_files2spark(spark, allfiles: bool = False, subdirs=('programs', 'das_framework', 'etc'), subdirs2root=(), dasmods=("ctools")):
    """
...
def table2hists(d, schema, housing_varname: str = None, units: bool = False):
    """
...
def int_wlog(s: str, name: str) -> int:
    """
