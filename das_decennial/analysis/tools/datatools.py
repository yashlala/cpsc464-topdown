...
from das_constants import CC as con
...
class Analysis:
    def __init__(self, spark, save_location, analysis_script_path):
        ...
        save_location = "user007/analysis_results/logname_and_other_info/"

        which, for save_location_s3, will become
        "${DAS_S3INPUTS}/users/user007/analysis_results/logname_and_other_info/"

        and for save_location_linux
        "/mnt/users/user007/analysis_results/logname_and_other_info/"
        """
...
class PickledDASExperiment:
    ...
    def getDF(self, mapper_type="sparse", add_metadata_columns=['run_id', 'plb', 'budget_group'], rebuild=False):
        ...
        selection = [AC.GEOCODE, AC.RUN_ID, AC.PLB, AC.BUDGET_GROUP] + self.schema.dimnames + [AC.ORIG, AC.PRTCTD]
...
def getJoinedDF(priv_experiment, orig_experiment):
    ...
    - Need to join orig with protected one data set (run, plb, budget_group) at a time
    - Good news, this basically already happens since we already transform each one individually
      and then union them!
    1. Access and generate each protected df
    2. Get the orig df
    3. Join each protected df with the orig df
    4. Union all dfs
    """
    ...
    column_order = [AC.GEOCODE, AC.RUN_ID, AC.PLB, AC.BUDGET_GROUP] + priv_experiment.schema.dimnames + [AC.ORIG, AC.PRTCTD]
...
def getMicrodataDF(spark, dasrun, schema, protected=True, metadata_columns=[AC.PLB, AC.RUN_ID, AC.BUDGET_GROUP], mangled_names=True, recoders=None):
    """
    ...
    protected : bool
        If True, then create records from the protected data (i.e. 'syn' or AC.PRTCTD; aka DAS output)
    ...
    rdd = spark.sparkContext.pickleFile(dasrun.data_path)
    df = (
        rdd.flatMap(lambda node: mappers.getMicrodataDF_mapper(node, schema, protected=protected, mangled_names=mangled_names, recoders=recoders))
           .map(lambda rowdict: Row(**rowdict))
           .toDF()
           .persist()
    )
