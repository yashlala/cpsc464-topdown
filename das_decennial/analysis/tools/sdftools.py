...
def aggregateGeolevels(spark_session, spark_df, geolevels, grfc_path, prim_crosswalk={}, strong_mcd_states=CC.STRONG_MCD_STATES_DEFAULT,
                       aian_areas=CC.DAS_AIAN_AREAS_CNSTAT, aian_ranges_path=CC.AIAN_RANGES_PATH_DEFAULT,
                       fed_airs=AC.FED_AIRS, verbose=True, return_dict=False):
    """
    ...
    if not np.all([x in spark_df.columns for x in geolevels]):
        ...
        crossdf = crosswalk.getCrosswalkDF(spark_session, join_geolevels, grfc_path, strong_mcd_states, aian_areas,
                                           aian_ranges_path, fed_airs, prim_crosswalk=prim_crosswalk)
        if verbose:
    ...
    for i, geolevel in enumerate(geolevels):
        ...
        if return_dict:
            if i == 0:
                dfdict = {geolevel: geodf}
            else:
                dfdict.update({geolevel: geodf})
        else:
            df = geodf if i == 0 else df.unionByName(geodf)
            df = df.persist()
    ...
    return dfdict if return_dict else df
...
def answerQuery(spark_df, schema, queryname, labels=False, merge_dims=False, verbose=True):
    ...
    - this function will group by all non-numeric columns, meaning that it will aggregate over each of the numeric columns
        - this sometimes makes sense (i.e. for 'orig' and 'protected'), but sometimes may not (i.e. 'L1')
    """
...
def buildGeolatticeTrunkColumns(spark_df, geolevels):
    ...
    for geolevel in geolevels:
        if geolevel == CC.US:
            spark_df = spark_df.withColumn(CC.US, sf.col(CC.US)).persist()
        else:
...
def stripSQLFromColumns(sparkdf, ignore=None):
    ...
    e.g. after computing a sum over geocode and an average over runs, we might have:
         "avg(sum(protected))" as one of the column names
         we use this function to strip away everything except for the AC.PRTCTD that's flanked
...
def getFullWorkloadDF(df, schema, workload, groupby=(AC.GEOCODE, AC.GEOLEVEL, AC.RUN_ID, AC.PLB, AC.BUDGET_GROUP)):
    ...
    # 9. create new columns for the missing_df (was_missing = "1", orig = 0, protected = 0)
    ...
    if not missing_rdd.isEmpty():
        ...
        missing_df = missing_df.withColumn(AC.PRTCTD, sf.lit(0))
...
def getTableCV(df, run_id, groupby=(AC.GEOCODE, AC.GEOLEVEL, AC.LEVEL, AC.QUERY), column=AC.PRTCTD, colname="CV"):
    ...
    # grab the first run's protected count and perform max(1,first_run[AC.PRTCTD])
    ...
    ###first_run = drop(first_run, [x for x in first_run.columns if x not in ['join_key', 'max(1,protected)']]).persist()
    ...
    # calcualate CV = var_samp(protected) / max(1,protected)
...
def getL1(df, colname="L1", col1=AC.PRTCTD, col2=AC.ORIG):
    return df.withColumn(colname, sf.abs(sf.col(col1) - sf.col(col2))).persist()


def getL2(df, colname="L2", col1=AC.PRTCTD, col2=AC.ORIG):
    return df.withColumn(colname, sf.pow(sf.col(col1) - sf.col(col2), sf.lit(2))).persist()


def mapL1Relative(grouped_values):
    # format on input is:
    # ((protected_query_val1, orig_query_val1, protected_sum, orig_sum), (protected_query_val2, orig_query_val2, protected_sum, orig_sum), ...)
    protected_vals = []
    orig_vals = []
    for row in grouped_values:
        protected_vals.append(row[0])
        orig_vals.append(row[1])
        protected_sum = row[2]
        orig_sum = row[3]

    max_col_ind = np.argmax(protected_vals)
    result = [int(protected_sum), int(orig_sum)]

    # Note that the metric requires both of the sums above to be non-zero. When all elements of protected_vals are correctly
    # protected_vals are correctly zero, define L1Relative to be zero. Also, when either protected_sum or orig_sum is zero and the
    ...
    if orig_sum == 0. and protected_sum == 0.:
        result.append(0.)
    elif orig_sum > 0. and protected_sum > 0.:
        result.append(float(np.abs(protected_vals[max_col_ind] / protected_sum - orig_vals[max_col_ind] / orig_sum)))
    else:
        result.append(2.)
    return result

def getL1Relative(df, colname="L1Relative", col1=AC.PRTCTD, col2=AC.ORIG, denom_query="total", denom_level="total"):
    ...
    # Format: (tuple_key, ((protected_query_val, orig_query_val, query_name))
    ...
    # Format: (tuple_key, (protected_sum, orig_sum))
    ...
    # Format: (tuple_key, ((protected_query_val, orig_query_val, query_name), (protected_sum, orig_sum)))
    ...
    # Format: (tuple_key_with_query_name, (protected_query_val, orig_query_val, protected_sum, orig_sum))

    grouped_rdd = rdd.groupByKey()
    # Format: (tuple_key_with_query_name, ((protected_query_val1, orig_query_val1, protected_sum, orig_sum), ...))
...
def getSignedError(df, colname="signed_error", col1=AC.PRTCTD, col2=AC.ORIG):
    return df.withColumn(colname, sf.col(col1) - sf.col(col2)).persist()


def getNonzero(df, groupby=[AC.RUN_ID, AC.GEOCODE, AC.GEOLEVEL], cols=[AC.PRTCTD, AC.ORIG]):
    for col in cols:
...
def getCellDensity(df, schema, groupby=[AC.RUN_ID, AC.GEOCODE, AC.GEOLEVEL], cols=[AC.PRTCTD, AC.ORIG]):
...
def getCellSparsity(df, schema, groupby=[AC.RUN_ID, AC.GEOCODE, AC.GEOLEVEL], cols=[AC.PRTCTD, AC.ORIG]):
    df = getNonzero(df, groupby=groupby, cols=cols)
    for col in cols:
...
def getGeounitTVD(df, colname="1-TVD", col1=AC.PRTCTD, col2=AC.ORIG, groupby=[AC.GEOCODE, AC.GEOLEVEL, AC.RUN_ID, AC.BUDGET_GROUP, AC.PLB]):
    """
...
def getGeolevelTVD(df, colname="1-TVD", col1=AC.PRTCTD, col2=AC.ORIG, groupby=[AC.GEOLEVEL, AC.RUN_ID]):
    # L1 error for each cell
    df = getL1(df, colname="L1", col1=col1, col2=col2)
...
def getAvgSignedErrorByTrueCountRuns(df, bins=[0,1,10,100,1000,10000],
    ...
    df = getSignedError(df, colname="signed_error", col1=AC.PRTCTD, col2=AC.ORIG).persist()
...
def getAvgAbsErrorByTrueCountRuns(df, bins=[0,1,10,100,1000,10000],
    ...
    df = getL1(df, colname="abs_error", col1=AC.PRTCTD, col2=AC.ORIG).persist()
...
def getValidGeounitTrials(catdf):
    ...
    filtered_totals = totals.filter((sf.col(AC.PRTCTD) > 0) & (sf.col(AC.ORIG) > 0)).persist()
...
def getAgeQuantilesAvgL1(valid_catdf, schema, labels):
    ...
    df = df.select([AC.GEOLEVEL, AC.GEOCODE, AC.RUN_ID, AC.PLB, AC.QUERY, 'percentile', AC.ORIG, AC.PRTCTD]).persist()
    df = df.withColumn("quantile_L1", sf.abs(sf.col(AC.PRTCTD) - sf.col(AC.ORIG))).persist()
...
def getRowGroupQuantilesOverPersonAges(rows, schema, quantiles, labels):
    """
    creates microdata from the row counts and calcualtes the quantiles across the ages in the 'orig' microdata and the 'protected' microdata
    ex)
        rows = [
           { 'age':   '1', 'orig': '2', 'protected': '3' },
           { 'age':  '25', 'orig': '3', 'protected': '1' },
           { 'age': '115', 'orig': '1', 'protected': '3' }
        ]
        ...
        quantile | orig | protected | ...,

        where ... are the other columns indicating things like plb, geocode, etc.
        and orig/protected indicate the quantiles for age (e.g. median age)
    ...
    for row in rows:
        ...
        protected_microdata += [row['age']]*row[AC.PRTCTD]
    ...
    if labels:
        ...
        protected_microdata = [age_leveldict[x] for x in protected_microdata]
    ...
    protected_microdata = np.array(protected_microdata).astype('int')

    orig_quantiles = np.quantile(orig_microdata, quantiles)
    protected_quantiles = np.quantile(protected_microdata, quantiles)

    cols = {}
    cols[AC.ORIG] = orig_quantiles
    cols[AC.PRTCTD] = protected_quantiles
    cols['quantile'] = [str(x) for x in quantiles]
...
def getCellSparsityByGroup(df, schema, groupby=[AC.GEOCODE, AC.GEOLEVEL, AC.PLB, AC.RUN_ID, AC.BUDGET_GROUP, AC.QUERY]):
    ...
    df = df.groupBy(groupby).agg(
        ...
        count_nonzeros(sf.col(AC.PRTCTD) > 0).alias(f'{AC.PRTCTD}_num_nonzero_cells')
    ).persist()
...
def getCellSparsityAcrossGeolevelSuperHistogram(df, groupby=[AC.GEOLEVEL, AC.PLB, AC.BUDGET_GROUP, AC.QUERY, AC.RUN_ID]):
    ...
    df = df.withColumn(f"{AC.PRTCTD}_geolevel_sparsity", 1.0 - (sf.col(f'{AC.PRTCTD}_num_nonzero_cells') / sf.col('num_cells'))).persist()
    df = df.withColumn("L1(geolevel_sparsity)", sf.abs(sf.col(f"{AC.PRTCTD}_geolevel_sparsity") - sf.col(f"{AC.ORIG}_geolevel_sparsity"))).persist()
    return df
...
{AC.S3_BASE}user007/Aug29_experiments_hierarchicalAgeRangeTopDown_branchFactor4_output_danVariant1-2/td4/run_0000/)
...
def population_density(spark, df, schema, geolevels, grfc_path):
    geolevels = du.aslist(geolevels)

    # get the geounit totals for each geounit in all geographic levels provided
    df = aggregateGeolevels(spark, df, geolevels, grfc_path)
    ...
    crossdf = crosswalk.getCrosswalkDF(spark, geolevels, grfc_path)
    ...
    for col in [AC.ORIG, AC.PRTCTD]:
        df = df.withColumn(f"pop_density_{col}", sf.col(col) / sf.col("AREALAND")).persist()
    df = df.withColumn("diff_pop_density", sf.col(f"pop_density_{AC.PRTCTD}") - sf.col(f"pop_density_{AC.ORIG}")).persist()
...
#############################################################################
def getAnswers(spark, df, geolevels, schema, queries, grfc_path):
    """ Answers queries at the geolevels specified """
    geoleveldf = aggregateGeolevels(spark, df, geolevels, grfc_path)
...
def votingMetricsSummary(spark, df, geolevels, queries, queries2, schema, grfc_path):
    # This computes race counts and provides summary statistics for specified district (upper/lower chambers, CD) using Some Human's metrics
    u=getAnswers(spark, df, geolevels, schema, queries, grfc_path)
    ...
    #u_df = u.df
    #v_df = v.df
    v = v.select(['geocode', 'geolevel', 'run_id', 'plb', AC.ORIG, AC.PRTCTD])
    v = v.withColumnRenamed(AC.ORIG, f'{AC.ORIG}_total')
    v = v.withColumnRenamed(AC.PRTCTD, f'{AC.PRTCTD}_total')
...
def votingMetricsAverage(joined_df):
    ...
    joined2_df = joined_df.withColumn(f'{AC.ORIG}_race_divided_by_{AC.ORIG}_total', sf.col(AC.ORIG) / sf.col(f'{AC.ORIG}_total'))
    avgd_df = joined2_df.groupBy([f'geocode', 'geolevel', 'plb', 'level', 'query']).avg()
    joined_df3 = joined2_df.withColumn(f'{AC.PRTCTD}_race_divided_by_{AC.PRTCTD}_total', sf.col(AC.PRTCTD) / sf.col(f'{AC.PRTCTD}_total'))
    ...
    final=avgd_df2.select(['geocode','geolevel','plb','level',f'avg({AC.ORIG}_race_divided_by_{AC.ORIG}_total)',f'avg({AC.PRTCTD}_race_divided_by_{AC.PRTCTD}_total)'])
    print("The average race counts in given district, averaged over trials are:")
    final.show()
    return final

def AvgPop(spark, df, geolevels, queries, schema, grfc_path):
...
def Variation(spark, df, geolevels, queries, schema, avgpopG, grfc_path):
    ...
    g=getSampleVariance(u,AC.ORIG,hilk,groupby=groupz)
    k=getSampleVariance(u,AC.PRTCTD,hilk,groupby=groupz)
    columns_to_drop = [f'sum({AC.ORIG})', f'sum({AC.PRTCTD})',f'sum({AC.ORIG}^2)',f'n*sum({AC.ORIG}^2)',f'sum({AC.ORIG})^2',f'n*sum({AC.ORIG}^2)-sum({AC.ORIG})^2']
    ga = g.drop(*columns_to_drop)
    columns_to_drop2 = [f'sum({AC.ORIG})', f'sum({AC.PRTCTD})',f'sum({AC.PRTCTD}^2)',f'n*sum({AC.PRTCTD}^2)',f'sum({AC.PRTCTD})^2',f'n*sum({AC.PRTCTD}^2)-sum({AC.PRTCTD})^2']
    ka=k.drop(*columns_to_drop2)
    cap=avgpopG.join(ga,on=groupz)
    cap2=cap.join(ka,on=groupz)
    cap3=cap2.withColumn('sqrt var', sf.sqrt(sf.col(f'sample_variance({AC.PRTCTD})')))
    #Relative Variation for each group over runs
    cap4=cap3.withColumn('relative var',sf.col('sqrt var')/sf.col('avg({AC.PRTCTD})'))
    return cap4
...
def VariationPop(spark,df,geolevels,queries,schema, grfc_path):
    # This is for Variation relative to Population.
    # This function calculates Relative Variation
    u=getAnswers(spark,df,geolevels,schema,queries, grfc_path)
    go = u.select(['geocode','geolevel','run_id','level',AC.ORIG,AC.PRTCTD])
    go2=go.withColumn('sq',sf.lit(2))
    ye=go2.withColumn('diff',sf.col(AC.PRTCTD)-sf.col(AC.ORIG))
    ...
    groupz=['geocode', 'geolevel', 'level']
    ye5=ye4.groupby(['geocode','geolevel','level',AC.ORIG]).agg(sf.round(sf.sum("variation for each run"),4).alias("TotVariation"))
...
def RelVarPop(ye5):
    ...
    ye6=ye5.withColumn('relative var', sf.sqrt(sf.col('TotVariation'))/sf.col(AC.ORIG))
    return ye6
...
def Coe_of_variation(sdf, RMS):
    ...
    if (sdf.count() > 0):
        avg_geographies = sdf.groupBy().avg(AC.ORIG).collect()[0][0]
...
def MAPE(sdf):
    ...
    number_of_rows = sdf.count()
    if (number_of_rows > 0):
        sdf=sdf.withColumn('abs diff div cef', sf.col('L1')/sf.col(AC.ORIG))
...
def MALPE(sdf):
    ...
    number_of_rows = sdf.count()
    if (number_of_rows > 0):
        sdf=sdf.withColumn('diff', sf.col(AC.PRTCTD)-sf.col(AC.ORIG))
        sdf=sdf.withColumn('diff div cef', sf.col('diff')/sf.col(AC.ORIG))
...
def combined_metrics(schema, sdf, spark, geolevels, agekey, sexkey, bucketkey, key, grfc_path):
...
, "85 to 89 years", "90 to 94 years", "95 to 99 years", "100 to 104 years", "105 to 109 years", "110 to 115\
 years"]
    ...
    if (key=="F"):
        ...
        spark_df1 = aggregateGeolevels(spark, sdf, geolevels, grfc_path)
        ...
        spark_df2 = aggregateGeolevels(spark, sdf, geolevels, grfc_path)
        ...
        spark_df1 = getL1(spark_df1, colname = "L1", col1=AC.PRTCTD, col2=AC.ORIG)
        spark_df1 = getL2(spark_df1, colname = "L2", col1=AC.PRTCTD, col2=AC.ORIG)
        spark_df2 = getL1(spark_df2, colname = "L1", col1=AC.PRTCTD, col2=AC.ORIG)
        spark_df2 = getL2(spark_df2, colname = "L2", col1=AC.PRTCTD, col2=AC.ORIG)
        spark_df1.show(100, False)
#        spark_df2.show(100, False)
        spark_df2=spark_df2.filter(spark_df2.level.rlike("Not"))
        spark_df2.show(100, False)
        spark_df1=spark_df1.select("geocode",AC.ORIG,AC.PRTCTD,"level","L1","L2")
        spark_df2=spark_df2.select("geocode",AC.ORIG,AC.PRTCTD,"level","L1","L2")
        spark_df2=spark_df2.withColumnRenamed(AC.ORIG,f"{AC.ORIG}1").withColumnRenamed(AC.PRTCTD,f"{AC.PRTCTD}1").withColumnRenamed("L1","L1_1").withColumnRenamed("L2","L2_2")
        ...
        spark_df3=spark_df3.withColumn("fraction",sf.col(f"{AC.ORIG}1")/sf.col(AC.ORIG))
        spark_df3.show(100,False)
