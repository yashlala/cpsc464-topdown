...
def getDenseDF_mapper(node, schema):
    ...
    for c, cell in enumerate(np.ndindex(schema.shape)):
        ...
        rowdict['protected'] = int(protected[cell])
...
def getSparseDF_mapper(node, schema):
    ...
    key_dict = {'raw': AC.ORIG, 'syn': AC.PRTCTD}
    ...
    for ind in nz_indices:
        ...
        for key in data.keys():
            rowdict[key_dict[key]] = float(data[key][0,ind])
...
def getMicrodataDF_mapper(node, schema, protected=True, mangled_names=True, recoders=None):
