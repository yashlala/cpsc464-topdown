...
def test_getiter(df_das_stub):
    aml = driver.AbstractDASModule(config=config, name="lists", das=df_das_stub)
    ...
    am = driver.AbstractDASModule(config=config, name=None, das=df_das_stub)
    assert am.gettuple_of_ints("F", sep=r"[\W\,\;]{1}", section="lists") == (1, 2, 3, 4)


def test_getconf(df_das_stub):
    amv = driver.AbstractDASModule(config=config,name="values", das=df_das_stub)
...
def test_getconfitems(df_das_stub):
    am = driver.AbstractDASModule(config=config, das=df_das_stub, name='abm')
    assert am.getconfitems("values") == [('a', '1'), ('b', '2.'), ('c', 'true'), ('d', 'sssstttrrring'), ('e', ''), ('f', '')]


def test_gettuple_of_fraction2floats(df_das_stub):
    am = driver.AbstractDASModule(config=config, das=df_das_stub, name='fractions')
    assert am.gettuple_of_fraction2floats("A") == (0.1, 0.7555555555555555, 0.02)


def test_gettuple_of_fractions(df_das_stub):
    am = driver.AbstractDASModule(config=config, das=df_das_stub, name='fractions')
    assert tuple(f.denominator for f in am.gettuple_of_fractions("A")) == (10, 45, 50)
    assert tuple(f.numerator for f in am.gettuple_of_fractions("A")) == (1, 34, 1)
    assert tuple(f.denominator for f in am.gettuple_of_fractions("B")) == (5,)
    assert tuple(f.numerator for f in am.gettuple_of_fractions("B")) == (2,)
    with pytest.raises(ValueError, match="cannot be converted to Fraction"):
        am.gettuple_of_fraction2floats("C")


def test_getfraction(df_das_stub):
    am = driver.AbstractDASModule(config=config, das=df_das_stub, name='fractions')
    assert am.getfraction("B").numerator == 2
    assert am.getfraction("B").denominator == 5
    with pytest.raises(ValueError, match="cannot be converted to Fraction"):
        am.getfraction("C")

def test_set_parameter():
    test_config = HierarchicalConfigParser()
    test_config.read_file(io.StringIO(s_config))
    assert driver.get_parameter(test_config, "DEFAULT:defopt") == "defopt"
    driver.set_parameter(test_config, "DEFAULT:defopt:test")
    assert driver.get_parameter(test_config, "DEFAULT:defopt") == "test"


def test_get_parameter():
    test_config = HierarchicalConfigParser()
    test_config.read_file(io.StringIO(s_config))
    assert driver.get_parameter(test_config, "DEFAULT:defopt") == "defopt"                 # Exists in config
    assert driver.get_parameter(test_config, "DEFAULT:defopt:defopt") == "defopt"          # Exists in config
    assert driver.get_parameter(test_config, "DEFAULT:badargument:defopt") == "defopt"     # doesnt exist, will return default
