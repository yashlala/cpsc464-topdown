...
import sys
# Some Human commented these out, because their action determines on where py.test is run from.
...
def df():
    ...
    submodules = ["ctools", "certificate", "dfxml/python/dfxml"]
    ...
    for submodule in submodules:
        ...
        pyfiles = [fn for fn in glob.iglob('{}/**/*.py'.format(submodule_dir), recursive=True) if "test" not in fn]

        for fullname in pyfiles:
            zipf.write(fullname, arcname='das_framework/'+submodule.split('/')[-1]+fullname.split(submodule_dir)[1])
    ...
    for fullname in glob.glob(os.path.dirname(os.path.dirname(__file__))+'/*.py'):
        ...
        zipf.write(fullname, arcname=os.path.join('das_framework', os.path.basename(fullname)))
...
def test_noisealg_repr(df_das_stub):
    ...
    # Make dummy engine
    e = spark_sql_das_engine.engine(config=config, name='engine', das=df_das_stub)
...
@pytest.mark.skip(reason="broken needs update not needed for das_framework")
...
def test_engine_create_noisifiers(df_das_stub):
...
@pytest.mark.skip(reason="broken needs update not needed for das_framework")
def test_engine_nonoise(df, df_das_stub):
    ...
    protected_tables = e.run(original_data)[0]
    true_tables = original_data["true_tables"]

    for tname, table in protected_tables.items():
        # Check counts
        assert (table.as_matrix().shape[0] == 4)
        # Check that tables are identical
        assert (true_tables[tname].as_matrix() == protected_tables[tname].as_matrix()).all()
...
def test_engine_reproducible(df, df_das_stub):
    ...
    for tname, table in protected_tables.items():
        ...
        assert (table.as_matrix().shape[0] == 4)
        # Check that tables are identical
        assert sum(sum(abs(protected_tables[tname].as_matrix() - np.array(
...
def test_engine_urandom(df, df_das_stub):
    ...
    # Create the engine
    e1 = spark_sql_das_engine.engine(config=config, name='engine', das=df_das_stub)
    original_data = {'original_data': {'inputtable': df}}

    # Run the engine
    assert e1.willRun()

    protected_tables1 = e1.run(original_data)[0]

    for tname, table in protected_tables.items():
        # Check counts
        assert (table.as_matrix().shape[0] == 4)
        # Check that tables are identical
        assert sum(sum(abs(protected_tables[tname].as_matrix() - true_tables[tname].as_matrix()))) > 0.1
        assert sum(sum(abs(protected_tables[tname].as_matrix() - true_tables[tname].as_matrix()))) < 50
        assert sum(sum(abs(protected_tables[tname].as_matrix() - protected_tables1[tname].as_matrix()))) > 0.1
        assert sum(sum(abs(protected_tables[tname].as_matrix() - protected_tables1[tname].as_matrix()))) < 50
        assert sum((abs(protected_tables[tname].as_matrix() - true_tables[tname].as_matrix()) / protected_tables[tname].as_matrix())[:, 0]) < 0.1
