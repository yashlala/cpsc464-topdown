...
def uniform2Gamma4(x):
    ...
    """
    sq2 = math.sqrt(2)
    return fsolve(lambda z, x_scaled=math.pi * (x - .5): .25 * math.log((z ** 2 + z * sq2 + 1) / (z ** 2 - z * sq2 + 1)) + .5 * (
            math.atan(z * sq2 + 1) - math.atan(1 - z * sq2)) - x_scaled , x0=np.array(0), fprime=lambda z: sq2 / math.pi / (1 + z**4))[0]


class NoiseAlgorithm(driver.AbstractDASModule):
    def __init__(self, config: ConfigParser, varname: str, **kwargs) -> None:
        self.config = config
    ...
    def setvar(self, varname: str, section: str):
        """
