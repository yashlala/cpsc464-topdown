...
forstrlist_re = re.compile(r"FOR (\w+)[.](\w+)\s*STRIN(\s*[a-zA-Z0-9_,\s]+\s*)", re.I)


def decode_loop(s: str):
    ...
    # The last two only work properly when sorted. LIST should be ascending numerics. STRLIST gets sorted automatically
    # as one can see below, which means the order of execution may be different
    m = forlist_re.search(s)
    if m:
        l = list(map(lambda s: Decimal(s.strip()), m.group(3).split(',')))
        return(m.group(1), m.group(2),l[0], l[-1], l, "LIST")
    m = forstrlist_re.search(s)
    if m:
        l = sorted(list(map(lambda s: s.strip(), m.group(3).split(','))))
        return (m.group(1), m.group(2), l[0], l[-1], l, "STRLIST")
    return None


def build_loops(config: ConfigParser):
...
def increment_state(loops,state_):
    ...
    while rank < len(state):
        if loops[rank][5] == "MUL":
            ...
            state[rank] = loops[rank][4][loops[rank][4].index(state[rank])+1] if state[rank] < loops[rank][3] else np.inf
        elif loops[rank][5] == "STRLIST" :
            state[rank] = loops[rank][4][loops[rank][4].index(state[rank])+1] if state[rank] < loops[rank][3] else state[rank] + '_______________'
...
def substitute_config(*, config: ConfigParser,loops,state):
...
def draw_graph(*, config: ConfigParser,datax,datay):
...
def graph_data(*, config: ConfigParser):
...
def fname(config: ConfigParser, var: str, default: str) -> str:
...
def run_experiment(*, config: ConfigParser, callback) -> None:
