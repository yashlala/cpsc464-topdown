...
def user_data() -> Dict:
    try:
        return get_url_json("http://{IP_ADDR}/2016-09-02/user-data/")
...
def instance_identity() -> Dict:
    try:
        return get_url_json('http://{IP_ADDR}/latest/dynamic/instance-identity/document')
    except (TimeoutError, urllib.error.URLError) as e:
...
def ami_id():
    return get_url('http://{IP_ADDR}/latest/meta-data/ami-id')
...
def get_ipaddr():
    try:
        return get_url("http://{IP_ADDR}/latest/meta-data/local-ipv4")
    except (TimeoutError, urllib.error.URLError) as e:
