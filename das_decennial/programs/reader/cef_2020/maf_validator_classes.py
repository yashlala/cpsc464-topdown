...
class GRFC20_validator:
    ...
    @classmethod
    def is_valid_COUSUBFS(self,x):
        """County Subdivision Functional Status"""
        if x is None or x == "None":
            return False
        return (leftpad(x,1)==leftpad('F',1)) or (leftpad(x,1)==leftpad('G',1)) or (leftpad(x,1)==leftpad('B',1)) or (leftpad(x,1)==leftpad('A',1)) or (leftpad(x,1)==leftpad('I',1)) or (leftpad(x,1)==leftpad('N',1)) or (leftpad(x,1)==leftpad('S',1)) or (leftpad(x,1)==leftpad('C',1))
    @classmethod
    ...
    def validate_pipe_delimited(self,x):
        ...
        if self.is_valid_COUSUBFS(fields[20]) == False: return False
        if self.is_valid_SUBMCDFP(fields[21]) == False: return False
        if self.is_valid_SUBMCDNS(fields[22]) == False: return False
        if self.is_valid_ESTATEFP(fields[23]) == False: return False
        if self.is_valid_ESTATENS(fields[24]) == False: return False
        if self.is_valid_CONCITFP(fields[25]) == False: return False
        if self.is_valid_CONCITNS(fields[26]) == False: return False
        if self.is_valid_PLACEFP(fields[27]) == False: return False
        if self.is_valid_PLACENS(fields[28]) == False: return False
        if self.is_valid_PLACEFS(fields[29]) == False: return False
        if self.is_valid_AIANNHFP(fields[30]) == False: return False
        if self.is_valid_AIANNHCE(fields[31]) == False: return False
        if self.is_valid_AIANNHNS(fields[32]) == False: return False
        if self.is_valid_AIHHTLI(fields[33]) == False: return False
        if self.is_valid_TRIBALSUBFP(fields[34]) == False: return False
        if self.is_valid_TRIBALSUBCE(fields[35]) == False: return False
        if self.is_valid_TRIBALSUBNS(fields[36]) == False: return False
        if self.is_valid_TTRACTCE(fields[37]) == False: return False
        if self.is_valid_TBLKGRPCE(fields[38]) == False: return False
        if self.is_valid_ANRCFP(fields[39]) == False: return False
        if self.is_valid_ANRCNS(fields[40]) == False: return False
        if self.is_valid_UACE(fields[41]) == False: return False
        if self.is_valid_UATYP(fields[42]) == False: return False
        if self.is_valid_UR(fields[43]) == False: return False
        if self.is_valid_CD116FP(fields[44]) == False: return False
        if self.is_valid_CDCURFP(fields[45]) == False: return False
        if self.is_valid_VTDST(fields[46]) == False: return False
        if self.is_valid_SLDUST(fields[47]) == False: return False
        if self.is_valid_SLDLST(fields[48]) == False: return False
        if self.is_valid_ZCTA5CE(fields[49]) == False: return False
        if self.is_valid_SDELMLEA(fields[50]) == False: return False
        if self.is_valid_SDSECLEA(fields[51]) == False: return False
        if self.is_valid_SDUNILEA(fields[52]) == False: return False
        if self.is_valid_UGACE(fields[53]) == False: return False
        if self.is_valid_PUMA(fields[54]) == False: return False
        if self.is_valid_LWBLKTYP(fields[55]) == False: return False
        if self.is_valid_INTPTLAT(fields[56]) == False: return False
        if self.is_valid_INTPTLON(fields[57]) == False: return False
        if self.is_valid_AREALAND(fields[58]) == False: return False
        if self.is_valid_AREAWATER(fields[59]) == False: return False
        if self.is_valid_AREAWATERINLD(fields[60]) == False: return False
        if self.is_valid_AREAWATERCSTL(fields[61]) == False: return False
        if self.is_valid_AREAWATERGRLK(fields[62]) == False: return False
        if self.is_valid_AREAWATERTSEA(fields[63]) == False: return False
        if self.is_valid_CSAFP(fields[64]) == False: return False
        if self.is_valid_CBSAFP(fields[65]) == False: return False
        if self.is_valid_METDIVFP(fields[66]) == False: return False
        if self.is_valid_PCICBSA(fields[67]) == False: return False
        if self.is_valid_CNECTAFP(fields[68]) == False: return False
        if self.is_valid_NECTAFP(fields[69]) == False: return False
        if self.is_valid_NECTADIVFP(fields[70]) == False: return False
        if self.is_valid_PCINECTA(fields[71]) == False: return False
        if self.is_valid_ACT(fields[72]) == False: return False
        if self.is_valid_MEMI(fields[73]) == False: return False
        if self.is_valid_NMEMI(fields[74]) == False: return False
        if self.is_valid_OIDTABBLK(fields[75]) == False: return False
        return True
...
class GRFC20:
    ...
    def parse_pipe_delimited(self,line):
        fields = line.split('|')
        if len(fields)!=76:
            raise ValueError(f'expected 76 fields, found {len(fields)}')
        ...
        self.COUSUBFS        = fields[20]  # County Subdivision Functional Status
        self.SUBMCDFP        = fields[21]  # Subminor Civil Division (FIPS)
        self.SUBMCDNS        = fields[22]  # Subminor Civil Division (National Standard)
        self.ESTATEFP        = fields[23]  # Estate (FIPS)
        self.ESTATENS        = fields[24]  # Estate (National Standard)
        self.CONCITFP        = fields[25]  # Consolidated City (FIPS)
        self.CONCITNS        = fields[26]  # Consolidated City (National Standard)
        self.PLACEFP         = fields[27]  # Place (FIPS)
        self.PLACENS         = fields[28]  # Place (National Standard)
        self.PLACEFS         = fields[29]  # Place Functional Status
        self.AIANNHFP        = fields[30]  # AIANNH (FIPS)
        self.AIANNHCE        = fields[31]  # AIANNH (Census)
        self.AIANNHNS        = fields[32]  # AIANNH (National Standard)
        self.AIHHTLI         = fields[33]  # American Indian/Hawaiian Home Land Trust Land Indicator
        self.TRIBALSUBFP     = fields[34]  # American Indian Tribal Subdivision (FIPS)
        self.TRIBALSUBCE     = fields[35]  # American Indian Tribal Subdivision (Census)
        self.TRIBALSUBNS     = fields[36]  # American Indian Tribal Subdivision (National Standard)
        self.TTRACTCE        = fields[37]  # American Indian Tribal Tract
        self.TBLKGRPCE       = fields[38]  # American Indian Tribal Block Group
        self.ANRCFP          = fields[39]  # Alaska Native Regional Corporation (FIPS)
        self.ANRCNS          = fields[40]  # Alaska Native Regional Corporation (National Standard)
        self.UACE            = fields[41]  # Urban Area
        self.UATYP           = fields[42]  # Urban Area Type
        self.UR              = fields[43]  # Urban/Rural
        self.CD116FP         = fields[44]  # Congressional District (Decennial)
        self.CDCURFP         = fields[45]  # Congressional District (Current)
        self.VTDST           = fields[46]  # Voting District
        self.SLDUST          = fields[47]  # State Legislative District (Upper Chamber)
        self.SLDLST          = fields[48]  # State Legislative District (Lower Chamber)
        self.ZCTA5CE         = fields[49]  # ZIP Code Tabulation Area (5-Digit)
        self.SDELMLEA        = fields[50]  # School District (Elementary)
        self.SDSECLEA        = fields[51]  # School District (Secondary)
        self.SDUNILEA        = fields[52]  # School District (Unified)
        self.UGACE           = fields[53]  # Urban Growth Area
        self.PUMA            = fields[54]  # Public Use Microdata Area
        self.LWBLKTYP        = fields[55]  # Block Type
        self.INTPTLAT        = fields[56]  # Internal Point (Latitude)
        self.INTPTLON        = fields[57]  # Internal Point (Longitude)
        self.AREALAND        = fields[58]  # Area (Land)
        self.AREAWATER       = fields[59]  # Area (Water)
        self.AREAWATERINLD   = fields[60]  # Area (Water-Inland)
        self.AREAWATERCSTL   = fields[61]  # Area (Water-Coastal)
        self.AREAWATERGRLK   = fields[62]  # Area (Water-Great Lakes)
        self.AREAWATERTSEA   = fields[63]  # Area (Water-Territorial Sea)
        self.CSAFP           = fields[64]  # Combined Statistical Area
        self.CBSAFP          = fields[65]  # Metropolitan and Micropolitan Statistical Area
        self.METDIVFP        = fields[66]  # Metropolitan Division
        self.PCICBSA         = fields[67]  # CBSA Principal City Indicator
        self.CNECTAFP        = fields[68]  # New England City and Town Combined Statistical Area
        self.NECTAFP         = fields[69]  # New England City and Town Area
        self.NECTADIVFP      = fields[70]  # New England City and Town Area Division
        self.PCINECTA        = fields[71]  # NECTA Principal City Indicator
        self.ACT             = fields[72]  # Address Stringistic Type
        self.MEMI            = fields[73]  # CBSA Metropolitan/Micropolitan Status Indicator
        self.NMEMI           = fields[74]  # NECTA Metropolitan/Micropolitan Status Indicator
        self.OIDTABBLK       = fields[75]  # Tabulation Block Object ID
    ...
    def validate(self):
        ...
        if not GRFC20_validator.is_valid_COUSUBFS(self.COUSUBFS): return False
    ...
    def validate_reason(self):
        ...
        if not GRFC20_validator.is_valid_COUSUBFS(self.COUSUBFS): reason.append('COUSUBFS ('+str(self.COUSUBFS)+') out of range (F-F, G-G, B-B, A-A, I-I, N-N, S-S, C-C)')
    ...
    def SparkSQLRow(self):
        ...
        return Row(
            ...
            cousubfs=safe_str(self.COUSUBFS),
