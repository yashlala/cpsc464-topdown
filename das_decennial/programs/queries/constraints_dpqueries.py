...
class DPquery:
    ...
    """
    __slots__ = ["name", "query", "DPanswer", "_plb", "DPmechanism", "Var", "mechanism_sensitivity", "budget_id"]
    DPanswer: Union[np.ndarray, multiSparse, bytes]
    _plb: Union[Fraction, float]
    DPmechanism: str
    Var: float
    name: str
    ...
    # The following getters/setters are implemented so that _plb can be accessed both via "plb" and "epsilon". The latter only for compatibility
    @property
    def plb(self):
        return self._plb
    ...
    def __add__(self, other):
        ...
        assert self.plb == other.plb, f"Trying to add DPanswers of queries with different epsilons: {self.plb}, {other.plb}"
