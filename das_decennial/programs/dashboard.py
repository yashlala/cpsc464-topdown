...
#         if val:
#             return val
#     return dashboard_jcr.get_config(name)
...
def das_sqs_url():
    return census_getenv('DAS_SQS_URL')

def bcc_https_proxy():
    val=census_getenv('BCC_HTTPS_PROXY')
    if len(val) == 0:
        return False
    else:
        return val
...
def das_object_cache_loc():
    return mkpath('${DAS_S3MGMT}', '${DAS_OBJECT_CACHE_BASE}')


def das_object_cache_acl():
    return census_getenv('DAS_S3MGMT_ACL')

def geturl(path):
    url=census_getenv("DAS_DASHBOARD_URL")
    if path:
        return "%s%s"%(url,path)
    else:
        return url
...
def sqs_queue():
    url=census_getenv("DAS_SQS_URL")
    endpoint=census_getenv("DAS_SQS_ENDPOINT")
    proxy=census_getenv("BCC_HTTPS_PROXY")
    config=False

    if endpoint:
        resource=boto3.resource('sqs',endpoint_url=endpoint)
    elif proxy:
        proxies={'https': proxy.replace("https://","")}
        config=botocore.config.Config(proxies=proxies)
        resource=boto3.resource('sqs',config=config)
    else:
        resource=boto3.resource('sqs')

    return resource.Queue(url)
...
def send_obj(*, obj, sender=DASLOG_SENDER):
    ...
    if (USE_DEBUG_URL in obj) or (os.getenv(FORCE_DEBUG_URL)==YES):
        url = census_getenv('DAS_LOG_ENDPOINT_DEBUG')
    else:
        url = census_getenv('DAS_LOG_ENDPOINT')

    if TRY_REST_FIRST not in os.environ:
        os.environ[TRY_REST_FIRST] = YES

    if os.getenv(TRY_REST_FIRST) == YES and url and len(url) > 0 and url != "none":
        ...
        if r:
            return
        #If REST fails once, we don't want to use SQS for the rest of the run,
