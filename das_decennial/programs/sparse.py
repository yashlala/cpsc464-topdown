...
class multiSparse:
    ...
    def __init__(self, array, shape=None):
        ...
        if isinstance(array, np.ndarray): # a numpy ndarray is expected to initialize multiSparse
            ...
            self.shape = shape
        elif array is None and shape is not None:
            self.sparse_array = ss.csr_matrix((1,np.prod(shape)), dtype=int)
            self.shape = shape
        else:
            raise TypeError("array must be of class numpy.ndarray; or, if shape is provided, either of type ss.csr_matrix, or None to create array")
