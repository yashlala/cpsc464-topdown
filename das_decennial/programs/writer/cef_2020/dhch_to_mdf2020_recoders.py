...
class DHCHToMDF2020HouseholdRecoderTen3Lev(MDF2020Recoder):

    schema_name = CC.SCHEMA_DHCH_TEN_3LEV
    ...
    def recode(self, row: Row):
        ...
        try:
            ...
            hhtype: int = int(row[self.getMangledName(CC.ATTR_HHTYPE_DHCH)])
            hhelderly: int = int(row[self.getMangledName(CC.ATTR_HHELDERLY)])
            hhtenshort_3lev: int = int(row[self.getMangledName(CC.ATTR_HHTENSHORT_3LEV)])
            ...
            # Set tenure to either owned with mortgage, owned without mortgage, or rented.
            # Will be reset to greedy detailed tenure after recoding has completed.
            # See dhch_to_mdf2020_writer.py's greedyTenureRecode method
            output_dict[CC.ATTR_MDF_TEN] = str(hhtenshort_3lev + 1)
...
class DHCHToMDF2020UnitRecoderTen3Lev(DHCHToMDF2020UnitRecoder):
    schema_name = CC.SCHEMA_DHCH_TEN_3LEV

class DHCHToMDF2020HouseholdRecoderFullTenure(DHCHToMDF2020HouseholdRecoder):
    schema_name = CC.SCHEMA_DHCH_FULLTENURE
    ...
    def recode(self, row):
        ...
        try:
            ...
            hhtype: int = int(row[self.getMangledName(CC.ATTR_HHTYPE_DHCH)])
            hhelderly: int = int(row[self.getMangledName(CC.ATTR_HHELDERLY)])
            tenure: int = int(row[self.getMangledName(CC.ATTR_TENURE)])
            ...
            # Will be reset to greedy detailed tenure after recoding has completed.
            # See dhch_to_mdf2020_writer.py's greedyTenureRecode method
            output_dict[CC.ATTR_MDF_TEN] = str(tenure + 1)
...
def zeropad(x, width: int) -> str:
    return '0'*(width-len(str(x)))+str(x)


def rtype_recoder(relgq) -> str:
    _HOUSING_UNIT = "3"
    return _HOUSING_UNIT


def schema_type_code_recoder() -> str:
    """
...
def schema_build_id_recoder() -> str:
    """
