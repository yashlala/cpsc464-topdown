...
# Automatically generated on Fri Oct 30 14:46:58 2020 by /mnt/users/user007/new-das-vm/das-vm-config/das_decennial/das_framework/ctools/schema/table.py
...
class MDF_Person_validator:
    ...
    def validate_pipe_delimited(self,x):
        ...
        if self.is_valid_SCHEMA_TYPE_CODE(fields[0]) == False: return False
        if self.is_valid_SCHEMA_BUILD_ID(fields[1]) == False: return False
        if self.is_valid_TABBLKST(fields[2]) == False: return False
        if self.is_valid_TABBLKCOU(fields[3]) == False: return False
        if self.is_valid_TABTRACTCE(fields[4]) == False: return False
        if self.is_valid_TABBLKGRPCE(fields[5]) == False: return False
        if self.is_valid_TABBLK(fields[6]) == False: return False
        if self.is_valid_EPNUM(fields[7]) == False: return False
        if self.is_valid_RTYPE(fields[8]) == False: return False
        if self.is_valid_GQTYPE(fields[9]) == False: return False
        if self.is_valid_RELSHIP(fields[10]) == False: return False
        if self.is_valid_QSEX(fields[11]) == False: return False
        if self.is_valid_QAGE(fields[12]) == False: return False
        if self.is_valid_CENHISP(fields[13]) == False: return False
        if self.is_valid_CENRACE(fields[14]) == False: return False
        if self.is_valid_LIVE_ALONE(fields[15]) == False: return False
        return True
...
class MDF_Unit_validator:
    ...
    def validate_pipe_delimited(self,x):
        ...
        if self.is_valid_TABBLKGRPCE(fields[5]) == False: return False
        if self.is_valid_TABBLK(fields[6]) == False: return False
        if self.is_valid_RTYPE(fields[7]) == False: return False
        if self.is_valid_GQTYPE(fields[8]) == False: return False
        if self.is_valid_TEN(fields[9]) == False: return False
        if self.is_valid_VACS(fields[10]) == False: return False
        if self.is_valid_HHSIZE(fields[11]) == False: return False
        if self.is_valid_HHT(fields[12]) == False: return False
        if self.is_valid_HHT2(fields[13]) == False: return False
        if self.is_valid_CPLT(fields[14]) == False: return False
        if self.is_valid_UPART(fields[15]) == False: return False
        if self.is_valid_MULTG(fields[16]) == False: return False
        if self.is_valid_THHLDRAGE(fields[17]) == False: return False
        if self.is_valid_THHSPAN(fields[18]) == False: return False
        if self.is_valid_THHRACE(fields[19]) == False: return False
        if self.is_valid_PAOC(fields[20]) == False: return False
        if self.is_valid_TP18(fields[21]) == False: return False
        if self.is_valid_TP60(fields[22]) == False: return False
        if self.is_valid_TP65(fields[23]) == False: return False
        if self.is_valid_TP75(fields[24]) == False: return False
        if self.is_valid_PAC(fields[25]) == False: return False
        if self.is_valid_HHSEX(fields[26]) == False: return False
        return True
