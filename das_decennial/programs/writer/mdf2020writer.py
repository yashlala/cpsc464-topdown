...
class MDF2020Writer(DASDecennialWriter):
    ...
    def saveRDD(self, path: str, df: DataFrame):
        """
        Saves the DataFrame.
        Coalesces it to a 100 partitions (with shuffle, so .repartition()) if self.s3cat is on.
        100 is based on a ballpark estimation for "optimal" partition size. Too many can lead to problems with bash command calling s3cat,
        ...
        """
        if self.s3cat:
            df = df.coalesce(CC.MDF_PARTITIONS_FOR_S3CAT)
        # df = self.sort(df)  # Note that sorting does not preserve partitioning
...
class DHCP_MDF2020_Writer(MDF2020Writer):
    ...
    def transformRDDForSaving(self, rdd: RDD):
        ...
        def addIndexAsEPNUM(row_index) -> Row:
            ...
            rowdict = row.asDict()
            rowdict['EPNUM'] = index
            ordered_cols = self.var_list + [CC.PRTCTD]
        ...
        spark_schema = StructType([StructField(col, (StringType() if col != 'EPNUM' else LongType())) for col in self.var_list + [CC.PRTCTD]])
...
def addEmptyAndGQ(node: Union[GeounitNode, dict],
    ...
    hu_gqtype = getNodeAttr(node, CC.INVAR)['gqhh_vect']
    ...
    unit_rowdict[CC.GEOCODE] = getNodeAttr(node, CC.GEOCODE)
...
class MDF2020H1HistogramWriter(MDF2020PersonWriter):
    ...
    def transformRDDForSaving(self, rdd: RDD):
        ...
        # df = rdd.toDF()
        spark_schema = StructType([StructField(col, StringType()) for col in self.var_list + [CC.PRTCTD]])
        spark = SparkSession.builder.getOrCreate()
        df = spark.createDataFrame(rdd, schema=spark_schema)
        df = df.select(self.var_list)
...
def addGroupQuarters(node: Union[GeounitNode, dict],
    ...
    unit_total = getNodeAttr(node, CC.INVAR)['gqhh_tot']

    # Number of group quarters is the total minus the number of households
    group_quarters = unit_total - sum([row[CC.PRTCTD] for row in rows])
    ...
    if to_microdata:
        ...
        unit_rowdict[CC.PRTCTD] = int(group_quarters)
