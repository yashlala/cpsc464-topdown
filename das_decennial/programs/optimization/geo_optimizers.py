...
class L2GeoOpt(GeoOptimizer):
    ...
    def __init__(self, *, NoisyChild=None, noisy_child_weights=None, DPqueries=None, nnls=True,
        ...
        use_postprocess_heuristics = self.getboolean(CC.USE_POSTPROCESS_HEURISTICS, section=CC.GUROBI, default=False)
        if not use_postprocess_heuristics:
            assert len(postprocess_only_constr_names) == 0
        self.constraints = [constr for constr in self.constraints if constr.name not in postprocess_only_constr_names]
        print(f"In geo_optimizers.py, after removing the postprocessing only constraints, which are {postprocess_only_constr_names}, self.constraints is: {[constr.name for constr in self.constraints]}")
    ...
    def getNZSubsets(self) -> Tuple[None, np.ndarray, List[int], Tuple[Union[np.ndarray, None], ...], np.ndarray, Union[None, np.ndarray]]:
        ...
        """
        n_list, parent_mask, parent_sub = self.findDesiredIndices(self.parent_shape, arrays=self.parent)
    ...
    def addDPQueriesToModel(self, model, two_d_vars, obj_fxn, parent_mask, q_set_list=None, sub_model_index=None, **kwargs):
        ...
        for ihist, dp_queries in enumerate(q_set_list):
            ...
            for st_dpq in dp_queries:
                ...
                dp_answer_all_children = np.hstack([ans[row_mask] for ans in st_dpq.DPanswerList]).ravel()  # Stack dp_answers for children into 1 vector
