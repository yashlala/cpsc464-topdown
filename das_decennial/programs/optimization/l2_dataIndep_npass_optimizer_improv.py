        ...
        for passIndex, passNum in enumerate(self.passNums):
            ...
            if(passNum > 0):
                ...
                for c in stacked_constraints_last_pass:
                    if c.name == CC.DETAILED:  # don't add a detailed constraint, unnecessary
                        continue
                    ...
                    # x = gb.MVar(np.array(two_d_vars[:, c.indices].tolist()).ravel("F"))  # Stack optimization vars for children into 1 vector
