...
def s3cat_download(prefix, *, demand_success=False, suffix="", verbose=False,
    ...
    :param get_sha:        if True, returns the sha256 of the catted file,
                           should be true, False by default for legacy reasons
    """
        ...
        with tempfile.NamedTemporaryFile(dir=get_tmp(), mode='wb') as tf:
            ...
            with tempfile.NamedTemporaryFile(mode='w') as script:
                ...
                subprocess.check_call(['/bin/bash', script.name],
                                      # Is this right? Do we want the script output to also go to tf?
                                      stdout=tf)

            proc = subprocess.check_output([ 'sha256sum', tf.name ])
            proc = proc.decode('utf-8').split()[0]
            with tempfile.NamedTemporaryFile(dir=get_tmp(), mode='w') as final:
                # We generate a header file
                if get_sha:
                    msg = "# SHA256 for the content below, DO NOT REMOVE "
                    msg += "(identifier used to trace original internal source file)\n"
                    print(msg)
                    final.write(msg)
                    final.write(f"# SHA256: {proc}\n")
                final.flush()
                subprocess.check_call(['cat',tf.name],stdout=final)
                # upload the combined file
                cmd = ['aws', 's3', 'cp', '--quiet', '--no-progress', final.name, prefix + suffix]
                verbprint(cmd)
                subprocess.check_call(cmd)
        if get_sha:
            return prefix + suffix, proc
        else:
...
def s3cat(prefix, *, demand_success=False, suffix="", verbose=False,
    ...
    :param get_sha - if True, return the sha256 of the catted file,
                     should usually be true, False by default for legacy reasons
    ...
    if not multipart:
        if get_sha:
            s3path, sha = s3cat_download(prefix, demand_success=demand_success, suffix=suffix, verbose=verbose, get_sha=get_sha)
        else:
            s3path = s3cat_download(prefix, demand_success=demand_success, suffix=suffix, verbose=verbose, get_sha=get_sha)
    else:
