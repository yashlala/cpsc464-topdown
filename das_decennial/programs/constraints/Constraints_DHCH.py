...
class ConstraintsCreator(querybase_constraints.AbstractConstraintsCreator):
    ...
    implemented = (
        "total",
        # TODO: Check that all struct zeros are implemented
        # "no_vacant",
        "living_alone",
        "size2",
        "size3",
    ...
    def h1(self, name):
        "Constrain to a previous H1 run"
        self.checkInvariantPresence(name, ('h1',))
        # HH_STATUS var: 0 - NIU (or GQ), 1 - Occupied, 2 - Vacant
        # Total households equal to total occupied units
        self.addToConstraintsDict(f"{name}_toth1occupied", "total", np.array(self.invariants["h1"][1].astype(int)), "=")
        # Number of vacant units
        self.addUnitConstraintsToDict(f"{name}_toth1vacant", CC.VACANT,  np.array(self.invariants["h1"][2].astype(int)), "=")
