...
"""
from programs.engine.rngs import DASRDRandIntegers, DASRandom
from dprng import discrete_gaussian, PerDrawOverflowStrategy, TotalOverflowStrategy, OverflowBoundError
...
_mostly_rng_factory = DASRandom  # RDRand draws wrapped in numpy.random module
...
class DPMechanism:
    ...
    _plb: Union[Fraction, float, None]
    variance: float
    sensitivity: float
    ...
    # The following getters and setters make sure that referral to either epsilon, rho or plb attribute would return/set _plb
    # The "epsilon"-named getters and setters should only be defined in subclasses that use epsilon, but it's done here for
    # backward compatibility.
    # The subclasses using rho may define _plb property named rho
...
class RationalGeometricMechanism(DPMechanism):
    ...
    def pmf(self, x: float, location: float = 0) -> float:
        """
            f(x) = p/(2-p) (1-p)^(abs(x))
        """
    ...
    def inverseCDF(self, quantile: float, location: float = 0.0) -> float:
        r"""
...
class RationalDiscreteGaussianMechanism(DPMechanism):
    ...
    def __init__(self, inverse_scale: Fraction, true_answer: np.ndarray):
        ...
        self.plb = inverse_scale
        self.sensitivity = 2.0  # Not used here, but constraints_dpqueries expects it and may be useful post factum (although it's l2 sensitivity that's relevant to this mechanism, and it's included in inverse_scale)
        ...
        overflows = None
        try:
            # TODO: do not hardcode neg_log_p_bound:
            perturbations, overflows = discrete_gaussian(self.sigma_sq, size=shape, strat=TotalOverflowStrategy(return_overflow_count=True, neg_log_p_bound=16))
            assert overflows == 0, f"Discrete Gaussian overflowed {overflows} times for query size {shape} and Sigma^2 {self.sigma_sq}"
        except (OverflowBoundError, AssertionError) as e:
            print(e)
            print("Sigma^2: ", self.sigma_sq)
            size = int(np.prod(shape))
            assert _called_from_test or _rng_factory is DASRDRandIntegers, "Only unit tests may override _rng_factory."
            rng = _rng_factory()
            perturbations = np.reshape(RationalSimpleDiscreteGaussian(sigma_sq=self.sigma_sq, size=size, rng=rng), shape)

        print(f"Query shape: {shape}, overflows: {overflows}, sigma^2: {self.sigma_sq}")

        self.protected_answer = true_answer + perturbations
...
def computeDiscreteGaussianVariance(sigma_sq) -> float:
    """
