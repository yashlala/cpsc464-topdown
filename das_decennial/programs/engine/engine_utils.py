...
class DASEngineHierarchical(AbstractDASEngine, metaclass=ABCMeta):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.level_partitions = self.gettuple_of_ints(CC.GEOLEVEL_NUM_PART, section=CC.ENGINE, default=str((0,)*len(self.all_levels))[1:-1])
        if len(self.level_partitions) != len(self.all_levels):
            raise DASValueError(f"The partition numbers for levels vector {self.level_partitions} ([engine]/geolevel_num_part) is not the same length as"
                                f"levels vector {self.all_levels}. No repartitioning will be performed", value=self.level_partitions)
        ...
        self.optimization_query_ordering = None
    ...
    def initializeAndCheckParameters(self) -> None:
        ...
        if self.save_noisy or self.postprocess_only or self.getboolean(CC.SAVEOPTIMIZED, default=True):
    ...
    def getNodesAllGeolevels(self, block_nodes: __NodeRDD__, additional_ic: bool = True) -> __NodeDict__:
        ...
        for level, upper_level, num_parts in zip(self.all_levels[:-1], self.all_levels[1:], self.level_partitions[1:]):
            ...
            if 'modified_block_geoids' in self.setup.__dict__.keys():
                all_nodes_dict[upper_level] = das_utils.repartitionNodesEvenly(all_nodes_dict[upper_level], num_parts, self.setup.modified_geoids_map[upper_level], is_key_value_pairs=False, partition_size=self.setup.part_size_noisy)
            elif num_parts > 0:
            ...
            # Checkpoint: save and reload by-level aggregated data
            if self.getboolean(CC.AGGREGATED_CHECKPOINT, default=True):
                spark = SparkSession.builder.getOrCreate()
                path = os.path.join(self.das.writer.output_path, "aggregated_cef_geonodes", f"{upper_level}.pickle")
                self.annotate("Saving aggregated CEF in geounit node format")
                das_utils.savePickledRDD(path, all_nodes_dict[upper_level], dvs_singleton=None)
                self.annotate("Reloading aggregated CEF in geounit node format")
                all_nodes_dict[upper_level] = spark.sparkContext.pickleFile(path)
    ...
    def makeOrLoadNoisy(self, nodes: __Nodes__) -> __Nodes__:
        ...
        if self.postprocess_only:
            ...
            if self.saveloc is not None:
                ...
                with tempfile.NamedTemporaryFile(dir=get_tmp(), mode='wb') as tf:
                    ...
                    with open(tf.name, 'r', newline='') as csvfile:
                        ...
                        widths = reader.__next__()
                        geolevels = reader.__next__()
                        self.das.reader.modified_geocode_dict = dict(zip([int(width) for width in widths], geolevels))
                        self.setup.modified_block_geoids = sorted([row[0] for row in reader])

                sc = SparkSession.builder.getOrCreate().sparkContext
                self.setup.modified_geoids_map = {}
                widths = sorted(list(self.das.reader.modified_geocode_dict.keys()), reverse=True)
                distinct_ids = self.setup.modified_block_geoids
                for width in widths:
                    level = self.das.reader.modified_geocode_dict[width]
                    if width != widths[0]:
            ...
            if self.saveloc is not None and 'modified_block_geoids' in self.setup.__dict__.keys():
                self.annotate("Saving modified block DAS geoids")
                block_geoids_path = os.path.join(self.saveloc, f"{self.app_id}-BlockGeoids.csv")
                with tempfile.NamedTemporaryFile(dir=get_tmp(), mode='wb') as tf:
                    with open(tf.name, 'w', newline='') as csvfile:
                        writer = csv.writer(csvfile)
                        widths = list(self.das.reader.modified_geocode_dict.keys())
                        geolevels = [self.das.reader.modified_geocode_dict[k] for k in widths]
                        writer.writerows([widths, geolevels])
                        writer.writerows([[row] for row in self.setup.modified_block_geoids])
                    subprocess.check_call(['aws', 's3', 'cp', '--quiet', '--no-progress', tf.name, block_geoids_path])
    ...
    def noisyPath(self, application_id: str, level: str, postfix: str) -> str:
    ...
    def makeDPNode(self, geounit_node: GeounitNode) -> GeounitNode:
        ...
        # create dictionaries for each histogram, each with format {query_name: query_object}:
        rounder_queries = [{}, {}]
        if self.optimization_query_ordering.rounder_query_names:
            for query_name in self.optimization_query_ordering.rounder_query_names[geounit_node.geolevel]:
                # TODO: replace try and except below with a permanent solution:
                try:
                    rounder_queries[0].update({query_name:self.setup.schema_obj.getQuery(query_name)})
                    # If query is not in the first histogram, check the other histogram:
                except AssertionError:
                    rounder_queries[1].update({query_name:self.setup.unit_schema_obj.getQuery(query_name)})
        for i in range(2):
            if len(rounder_queries[i]) == 0:
                rounder_queries[i] = None
    ...
    def getAdjustedInverseScalePerQuery(self, dpq_prop, geolevel_prop):
        ...
        if self.setup.privacy_framework in (CC.ZCDP,):
            return dpq_prop * geolevel_prop / self.budget.global_scale_sq
        elif self.setup.privacy_framework in (CC.PURE_DP,):
            return (dpq_prop * geolevel_prop) / self.budget.global_scale
        else:
    ...
    def addDPQ2Dict(self, query_set_name, query, dp_queries, dpq_prop, geolevel_prop, hist) -> None:
        ...
        dp_query = self.makeDPQuery(hist=hist, query=query, inverse_scale=inverse_scale)
    ...
    def logInvConsCheckPLB(self, nodes: __NodeRDD__) -> None:
        """ Log acting invariants and constraints and check budgets"""
        recs = nodes.map(GeounitNode.fromZipped).map(
            lambda node: (node.geolevel, ", ".join(map(str, node.invar.keys())), ", ".join(map(str, node.cons.keys())), node.getPLB())
        ).take(10)
        # rec: GeounitNode = GeounitNode.fromZipped(nodes.take(1)[0])
        rec = recs[0]
        gl = rec[0]
        # count = nodes.count()
        # self.log_and_print(f"Number nodes: {count}")
        self.log_and_print(f"{gl} invariants: {rec[1]}")
        self.log_and_print(f"{gl} constraints: {rec[2]}")
        plb_list = list(map(lambda d: d[-1], recs))
        conf_plb = float(self.budget.geolevel_prop_budgets_dict[gl] * self.budget.total_budget)
        ...
        if (abs(conf_plb - max_plb) > 1e-7) and (abs(conf_plb - min_plb) > 1e-7) and (abs(conf_plb - most_common_plb) > 1e-7):
            ...
            warnings.warn(f"Geolevel {gl} config-designated PLB does not correspond to minimal, maximal or most common PLB in {gl}")
