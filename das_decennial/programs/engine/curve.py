...
class BaseGeoCurve:
    ...
    """
    def __init__(self, geo_prop_int, query_prop_int, pure_eps, bounded: bool = True, dps: int = 50) -> None:
        """ Inputs:
    ...
    def get_delta(self, target_eps: float, tol: float = 0.0000001) -> float:
...
class UniformGeoCurve:
    ...
    def __init__(self, geo_prop, query_prop) -> None:
    ...
    def get_delta(self, target_eps: float, pure_eps: float, *, bounded: bool) -> float:
...
class BaseCurve:
    ...
    def get_hit(self, nscale, bounded: bool = True) -> float:
    ...
    def get_rho(self, nscale, bounded: bool = True) -> float:
        """
    ...
    def get_zcdp_delta(self, eps: float, nscale, bounded: bool = True) -> float:
        rho = self.get_rho(nscale, bounded)
        if self.verbose:
            print(f"For nscale {nscale}, eps {eps}, zCDP rho calculated as: {rho}")

        def g(alpha: float) -> float:  # Cannone et al Corollary 13 (zCDP => eps, delta-DP conversion for delta = g(alpha)); see (Eq 18)
        ...
        def gprime(alpha: float) -> float:  # Cannone et al Eq 19 (derivative of g; use to optimize over alpha in (1,infty) to approximate delta)
    ...
    def get_bw_delta(self, eps: float, nscale, bounded: bool = True) -> float:
        """
    ...
    def get_delta(self, eps: float, nscale, bounded: bool = True, zcdp: bool = False) -> float:
        if zcdp:
    ...
    def get_scale(self, eps: float, delta: float, bounded: bool = True, tol: float = 1e-7, zcdp: bool = False) -> float:
    ...
    def get_epsilon(self, delta: float, nscale, bounded: bool = True, tol: float = 1e-7, zcdp: bool = False) -> float:
        """
    ...
    def get_eps_equivalent(self, pure_dp_eps: float, delta: float, bounded: bool = True, zcdp: bool = False):
        """
    ...
    def get_curve(self, nscale, bounded: bool = True, start=None, end=None, plot=True, zcdp: bool = False):
        """
...
class zCDPEpsDeltaCurve(BaseCurve):
    """
        ...
        1/(p*g)^2.
    """
    def __init__(self, geo_allocations_dict, verbose: bool = True):
...
class GeographicScaleCurve(BaseCurve):
    ...
    """
    def __init__(self, *, geo_scale, query_prop) -> None:
...
def disc_vs_continuous(nscale: float = 0.35, bounded: bool = True) -> None:
...
def apples2apples_gauss_v_unif(pure_eps: float = 4.0):
...
def explore(pure_eps: float = 4.0, bounded: bool = True):
    # geo_ddp = [0.2, 0.2, 0.12, 0.12, 0.12, 0.12, 0.12]
    # query_ddp = [0.1, 0.2, 0.5, 0.05, 0.05, 0.05, 0.05]
...
def get_demo() -> zCDPEpsDeltaCurve:
    """
...
def get_demo2() -> zCDPEpsDeltaCurve:
    """
