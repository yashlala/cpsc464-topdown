...
def sample_bernoulli(p: Fraction, rng):
...
def sample_bernoulli_exp1(x: Fraction, rng):
...
def sample_bernoulli_exp(x: Fraction, rng):
    assert isinstance(x, Fraction)
    assert x >= 0
...
# assumes x is a rational number >= 0
def sample_geometric_exp_slow(x: Fraction, rng):
    assert isinstance(x, Fraction)
    assert x >= 0
    k = 0
    while True:
...
def sample_geometric_exp_fast(x: Fraction, rng):
    assert isinstance(x, Fraction)
...
# assumes scale>=0
def sample_dlaplace(scale, rng=None) -> float:
    if rng is None:
...
# assumes sigma2>=0
def sample_dgauss(sigma2, rng=None) -> int:
    if rng is None:
        rng = random.SystemRandom()
...
def normalizing_constant(sigma2) -> float:
    original = None
    poisson = None
    if sigma2 <= 1:
        original = 0
