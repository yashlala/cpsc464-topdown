...
class GeounitNode:
    ...
    # The constants labeled "from nodes.py" located in das_constants.py have to be the same as
    # attribute names (__slots__) and arguments of __init__ in the GeounitNode class below
    ...
    rounder_queries: List[Dict[str, AbstractLinearQuery]]
    ...
    def fromDict(cls, argdict: dict) -> 'GeounitNode':
        ...
        :return:
        """
        args_taken = [CC.RAW, CC.RAW_HOUSING, CC.GEOCODE, CC.GEOCODEDICT, CC.CONS, CC.INVAR, CC.DP_QUERIES]
        argdict[CC.CONS] = argdict.pop('_cons', None)
        argdict[CC.INVAR] = argdict.pop('_invar', None)
        args = {key: value for key, value in argdict.items() if key in args_taken}
        args['geocode_dict'] = args.pop(CC.GEOCODEDICT, None)
        return GeounitNode(**args)

    @classmethod
    def fromZipped(cls, zipped_node: Union[bytes, 'GeounitNode']) -> 'GeounitNode':
        """
        Unzips and unpickles the node
        """
        if isinstance(zipped_node, GeounitNode):
            return zipped_node
        return pickle.loads(zlib.decompress(zipped_node))

    @classmethod
    def clsToZipped(cls, unzipped_node: 'GeounitNode') -> bytes:
        """
        Pickles and zips the node passed as argument
        """
        if isinstance(unzipped_node, bytes):
            return unzipped_node
        return zlib.compress(pickle.dumps(unzipped_node))
    ...
    def dp(self) -> DPquery:
        ...
        if self.dp_queries is not None:
            try:
                return self.dp_queries[CC.DETAILED]
            except KeyError:
                pass
    ...
    def dp(self, value):
        ...
        """
        #self.dp_queries[CC.DETAILED] = value
    ...
    def querySets2Stack(self):
        ...
        for dpq_dict in (self.dp_queries, self.unit_dp_queries):
            if dpq_dict is not None:
                query_set_list.append({k: v for k, v in dpq_dict.items() if k != CC.DETAILED})
            else:
    ...
    def __eq__(self, other):
        ...
        """
        for attr in set(self.__slots__) - {'_invar', CC.SYN, CC.RAW, CC.RAW_HOUSING, CC.UNIT_SYN}:
    ...
    def __add__(self, other: 'GeounitNode', perform_attr_checks=True):
        """
        ...
        :return:
        """
        if perform_attr_checks:
            for attr in (CC.GEOLEVEL, CC.GEOCODEDICT, CC.PARENTGEOCODE):
                self.checkSameAttr(other, attr)

        sum_raw = self.sumAttr(other, CC.RAW)
        sum_raw_housing = self.sumAttr(other, CC.RAW_HOUSING)
        sum_syn = self.sumAttr(other, CC.SYN)
        sum_syn_housing = self.sumAttr(other, CC.UNIT_SYN)
    ...
    def sumAttr(self, other, attr):
        ...
        if selfattr is not None and otherattr is not None:
            ...
            msg = f"One of the GeoUnit-node addends has {attr} data, while the other does not ({CC.GEOCODE}s: {self.geocode} and {other.geocode})"
            logging.warning(msg)
    ...
    def setParentGeocode(self):
        ...
        """
        #TODO: deprecate
        mykeys = sorted([key for key in self.geocodeDict.keys()], reverse=True)
        ...
        try:
            ...
            raise DASValueError(
                f"Node has {CC.GEOCODE} {self.geocode}, but the {CC.GEOCODEDICT} {self.geocodeDict} does not refer to keys of length {geocode_len}", '')
    ...
    def getEpsilon(self):
        """
            ...
            TODO: This was supposed to be used if vacancy queries were accounted in different budget.
             Together with the need to rename epsilon->PLB/budget, will make a new method called getPLB
    ...
    def getPLB(self):
        plb = 0
        for dpq in tuple(self.dp_queries.values()) + tuple(self.unit_dp_queries.values()):
            plb += dpq.plb
        return plb
    ...
    def stackDetailedDPAnswers(self, parent_shape):
        ...
        """
        dpa = self.dp_queries[CC.DETAILED].unzipDPanswer().DPanswer
    ...
    def filterToNZ(self, nz):
        ...
        try:
            dpa = self.dp_queries[CC.DETAILED].unzipDPanswer().DPanswer
            self.dp_queries[CC.DETAILED].DPanswer = multiSparse.fromJK(nz, dpa.ravel()[nz], dpa.shape)
        except KeyError:
            pass
    ...
    def toDict(self, keep_attrs: Iterable[str]) -> dict:
        ...
        if keep_attrs is None:
            keep_attrs = (CC.GEOCODE, CC.RAW, CC.SYN)  # Default attributes to keep when saving
        node_dict = {}
    ...
    def checkConstraints(self, raw=True, return_list=False, constrs_to_skip=()):
        """
