...
class error_metrics(AbstractDASErrorMetrics):
    ...
    def run(self, geolevelRDD_dict: Dict) -> List[mgmod.MetricGroup]:
        """
        """
        config = self.config
...
class DASErrorMetrics():
    ...
    """
    def __init__(self, geolevelRDD_dict: Dict) -> None:
        """
    ...
    def getErrorMetric(self, geolevels: List[str] = None, metrics: List[str] = None, agg_types: List[str] = None,
                       data_pair: List[str] = None, summary_quantiles: List[float] = None) -> Dict:
        """
    ...
    def __getGeolevelRDDs(self, geolevels: List[str]) -> Dict:
        """
    ...
    def __getMetrics(self, rdd_dict, metrics, data_pair: List[str]):
        """
    ...
    def __aggregateByType(self, metric_rdd_dict, agg_types: List[str], summary_quantiles):
        """
        ...
        Outputs:
            a dictionary
        Notes:
            For AGG_SUM and AGG_AVG, all GeounitNodes are summed under the US_GEOCODE, regardless of whether
            the nodes span the entire US or not.
        """
        ...
        for rdd_key in list(metric_rdd_dict.keys()):
            ...
            for agg in agg_types:
                ...
                if agg == AGG_SUM:
                    ...
                    sum_error_metrics_node = (myrdd.map(lambda node: (US_GEOCODE, node))
                                                   .reduceByKey(sparkAggSum_ErrorMetricsNodes)
                                                   .map(lambda kv: kv[1])
                                                   .collect()
                                                   .pop()
                        )
                    # set geocode to be the "US level geocode", which is "0"
                    sum_error_metrics_node.geocode = US_GEOCODE
                    ...
                    # set geocode to be the "US-level geocode", which is "0"
                    avg_error_metrics_node.geocode = US_GEOCODE
                    # only provide the dictionary within the error metrics node
                    for em_key, em_val in sum_error_metrics_node.error_metrics.items():
                        rdd_results["{}.{}.{}".format(geolevel_key, agg, em_key)] = em_val
                    ...
                    if mycount == 1:
                        ...
                        collect_emnode_rdd = (myrdd.map(sparkAggCollect_MakeAllMetricsIntoLists)
                                                   .map(lambda node: (US_GEOCODE, node))
    ...
    def getRDDGeolevels(self) -> List[str]:
        """
...
def getSummaryStats(rdd, quantiles: List[float], max_collect: int = 25000) -> DataFrame:
    count = rdd.count()
    summary_stats = None
...
def getSummaryStats_DF(rdd, quantiles: List[float], quantile_approx_rel_err: float = 0.001) -> Dict:
    flatrdd = (rdd
...
def getSummaryStats_collect(rdd, quantiles: List[float]) -> Dict:
...
def getShapeFromMultiarrayDict(madict: Dict) -> tuple:
...
def reshapeQuantileMultiarraysInDict(qdict: Dict) -> Dict:
    summary_dict = {}
...
def quantileMultiarray(qdict: Dict, key: str) -> np.ndarray:
...
def multiarrayToKey(x: Dict) -> Dict:
...
def prepForSaving(mdict: Dict) -> Dict:
    newdict = {}
...
def errorWrapper(geo_node: GeounitNode) -> em.ErrorMetricsNode:
    """
